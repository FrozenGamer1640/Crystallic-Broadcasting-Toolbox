...
func get_body_parsed() -> Variant:
	...
	if(content_type == "application/twitch-api"):
		
		var data:Dictionary
		var key_and_value:Array
		var values:PackedStringArray
		
		body = body.replacen("+"," ")
		
		for body_part in body.split("&"):
			key_and_value = body_part.split("=")
			if(key_and_value[1].contains("%3")): values = key_and_value[1].split("%3")
			data[key_and_value[0]] = key_and_value[1] if values.is_empty() else values.duplicate()
